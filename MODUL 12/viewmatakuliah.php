<?php
  ...
  if (isset($_GET['search']) && !empty($_GET['search'])) {
      $search = $_GET['search'];
      $stmt = $con->prepare("SELECT * FROM t_matakuliah WHERE namaMatkul LIKE ? ORDER BY kodeMatkul ASC");
      $searchParam = "%$search%";
      $stmt->bind_param("s", $searchParam);
  } else {
      $stmt = $con->prepare("SELECT * FROM t_matakuliah ORDER BY kodeMatkul ASC");
  }
?>
<!DOCTYPE html>
<html lang="id">
                ...
                <?php
                  ...
                  if ($result->num_rows > 0) {
                      while ($data = $result->fetch_assoc())
                      {
                          ...
                          echo "<td>" . htmlspecialchars($data['namaMatkul']) . "</td>";
                          echo "<td>" . htmlspecialchars($data['sks']) . "</td>";
                          echo "<td>" . htmlspecialchars($data['jam']) . "</td>";
                          echo '<td class="action-links">
                              <a href="editmatakuliah.php?kodeMatkul='.$data['kodeMatkul'].'" class="btn btn-warning btn-sm">✏️ Edit</a>
                              <a href="hapusmatakuliah.php?kodeMatkul='.$data['kodeMatkul'].'" class="btn btn-danger btn-sm"
